...
import { sizeof, char, parse } from "structured-buffer";
import { readStreamAsync, parsePermissions, parseType } from "../common";

export const MAGIC = "070707";
...
export function* parseCPIO(input: Buffer, blockSize: number){
    ...
    while(true){
        ...
        const filesize = parseInt(parsed.c_filesize, 8);
        const name = (read(parseInt(parsed.c_namesize, 8))).toString("ascii").slice(0,-1);
        if(name === "TRAILER!!!"){
            break;
        }
        const content = read(filesize);
        const modeNum = parseInt(parsed.c_mode, 8);
        const type = parseType(modeNum);
        const majmin = parseInt(parsed.c_rdev, 8); // I think m_rdev is the binary format equivalent to h_majmin (PR IF THIS IS WRONG!)
        const entry: Entry = {
            name,
            modificationTime: new Date(parseInt(parsed.c_mtime, 8) * 1000),
            device: BigInt(parseInt(parsed.c_dev, 8)),
            inode: parseInt(parsed.c_ino, 8),
            ownerUID: parseInt(parsed.c_uid, 8),
            ownerGID: parseInt(parsed.c_gid, 8),
            size: filesize,
            mode: {
            ...
            content: type === "directory" ? null : // ternary operator madness 😢
                        (type === "blockDev" || type === "charDev") ?
                            { major: majmin & 0xFF, minor: majmin >>> 8 } 
                            : content,
        };
        yield entry;
    }
...
export async function* parseCPIOStream(input: Readable, blockSize: number){
    ...
    while(true){
        ...
        const filesize = parseInt(parsed.c_filesize, 8);
        const name = (await read(parseInt(parsed.c_namesize, 8))).toString("ascii").slice(0,-1);
        if(name === "TRAILER!!!"){
            break;
        }
